...
: "${OUTPUT_DIR:=/tmp/leona-device-attestation-e2e-$(date +%Y%m%d-%H%M%S)}"
...
if [[ -z "${ADB_SERIAL:-}" ]]; then
  ADB_SERIAL="$(adb devices | awk 'NR>1 && $2=="device" && $1 !~ /^emulator-/ { print $1; exit }')"
fi

if [[ -z "${ADB_SERIAL:-}" ]]; then
  echo "No connected physical Android device found. Set ADB_SERIAL explicitly." >&2
  exit 1
fi
...
setup_port_reverse() {
  ...
  adb -s "$ADB_SERIAL" reverse tcp:8090 tcp:8090 >/dev/null || true
}
...
const installId = `device-attestation-e2e-${Date.now()}`;
...
if (attestationMode === 'oem_debug_fake') {
  ...
  attestationToken = JSON.stringify({
    ...
    evidenceLabels: ['debug_fake', 'physical_device'],
    claims: { executionTarget: 'physical_device' },
    mode: 'oem_debug_fake',
  });
} else {
  attestationToken = JSON.stringify({
...
payload = {
    "deviceSerial": os.environ["ADB_SERIAL"],
...
echo "[Leona device attestation E2E] serial      : $ADB_SERIAL"
echo "[Leona device attestation E2E] output dir  : $OUTPUT_DIR"
echo "[Leona device attestation E2E] host base   : $LEONA_HOST_BASE_URL"
echo "[Leona device attestation E2E] app base    : $LEONA_REPORTING_ENDPOINT"
echo "[Leona device attestation E2E] mode        : $LEONA_SAMPLE_ATTESTATION_MODE"
...
printf '\n[Leona device attestation E2E] handshake response: %s\n' "$OUTPUT_DIR/handshake-response.json"
printf '[Leona device attestation E2E] report:             %s\n' "$OUTPUT_DIR/attestation-e2e-report.json"
