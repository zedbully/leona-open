...
/**
 * Request-only LPCARR01 V1 authenticated-body encoder.
 *
 * This deliberately produces only the opaque application body handed to the
 * Leo boundary.  It neither creates HTTP headers nor seals, opens, or retries
 * a Leo envelope.
 */
internal object LeonaProtectedPayloadCarrierV1 {
    const val MAX_PAYLOAD_BYTES = 131_072
    const val MAX_CARRIER_BYTES = 131_226

    private const val VERSION = 1
    private const val FIELD_COUNT = 5
    private const val TLV_HEADER_BYTES = 8
    private val MAGIC = "LPCARR01".toByteArray(StandardCharsets.US_ASCII)
    private val CODEC = LeonaEvidenceProtobufCodec.PAYLOAD_CODEC.toByteArray(StandardCharsets.US_ASCII)
    private val SCHEMA = LeonaEvidenceProtobufCodec.PAYLOAD_SCHEMA.toByteArray(StandardCharsets.US_ASCII)
    private val MESSAGE = LeonaEvidenceProtobufCodec.MESSAGE_TYPE.toByteArray(StandardCharsets.US_ASCII)
    ...
    sealed interface DecodeResult {
        data class Success(
            ...
            private val protobuf: ByteArray,
        ) : DecodeResult {
            val bytes: ByteArray get() = carrier.copyOf()
            val payload: ByteArray get() = protobuf.copyOf()
            val descriptor: LeonaProtectedPayloadDescriptor get() = LeonaEvidenceProtobufCodec.descriptor
        }
    ...
    enum class FailureCode {
        ...
        OVERSIZE, PROTOBUF_REJECTED, INTERNAL_ERROR, EXTERNAL_BLOCKED,
    }

    /** Encodes the already-strict Protobuf handoff into one canonical Leo body. */
    fun encode(handoff: LeonaProtectedLogicalPayloadHandoff): EncodeResult {
        if (handoff is LeonaProtectedLogicalPayloadHandoff.ExternalBlocked) {
            return EncodeResult.Failure(FailureCode.EXTERNAL_BLOCKED)
        }
        // Read once: typed handoffs expose defensive copies and must not be
        // sampled multiple times while composing the authenticated body.
        return encodePayload(handoff.bytes, handoff.descriptor)
    }

    /**
     * Single request-only composition entry: model validation and canonical
     * Protobuf encoding complete before their exact bytes are wrapped.
     */
    fun encodeRequest(
        request: LeonaEvidenceIngestRequestModel,
        appCapBytes: Int = LeonaEvidenceProtobufCodec.DEFAULT_APP_CAP_BYTES,
    ): EncodeResult = when (val protobuf = LeonaEvidenceProtobufCodec.encode(request, appCapBytes)) {
        is LeonaProtobufEncodeResult.Success -> encode(protobuf.handoff)
        is LeonaProtobufEncodeResult.Failure -> EncodeResult.Failure(FailureCode.PROTOBUF_REJECTED)
    }

    /** Internal seam: only [encode] accepts the public typed Protobuf handoff. */
    internal fun encodePayload(payload: ByteArray, descriptor: LeonaProtectedPayloadDescriptor): EncodeResult {
        if (!isCanonicalDescriptor(descriptor)) return EncodeResult.Failure(FailureCode.DESCRIPTOR_MISMATCH)
        ...
        writeTlv(output, 4, descriptor.schemaDigestSha256)
        writeTlv(output, 5, payload)
        val carrier = output.toByteArray()
        if (carrier.size != total || carrier.size > MAX_CARRIER_BYTES) {
            return EncodeResult.Failure(FailureCode.INTERNAL_ERROR)
        }
        return EncodeResult.Success(carrier)
    }

    /** Strict request-only parser used for deterministic local contract verification. */
    fun decodeRequest(carrier: ByteArray): DecodeResult {
        ...
        if (!codec.contentEquals(CODEC) || !schema.contentEquals(SCHEMA) || !message.contentEquals(MESSAGE) || digest.size != 32 || !digest.contentEquals(LeonaEvidenceProtobufCodec.descriptor.schemaDigestSha256)) {
        ...
        if (LeonaEvidenceProtobufCodec.decode(payload) !is LeonaProtobufDecodeResult.Success) return DecodeResult.Failure(FailureCode.PROTOBUF_REJECTED)
    ...
    private fun checkedTotal(payloadLength: Int): Int? {
        return try {
            ...
            var total = Math.addExact(MAGIC.size, 4) // version, count, reserved
            for (valueLength in intArrayOf(CODEC.size, SCHEMA.size, MESSAGE.size, 32, payloadLength)) {
                total = Math.addExact(total, TLV_HEADER_BYTES)
                total = Math.addExact(total, valueLength)
            }
    ...
    private fun isCanonicalDescriptor(descriptor: LeonaProtectedPayloadDescriptor): Boolean =
        descriptor.payloadCodec == LeonaEvidenceProtobufCodec.PAYLOAD_CODEC &&
            descriptor.payloadSchema == LeonaEvidenceProtobufCodec.PAYLOAD_SCHEMA &&
            descriptor.messageType == LeonaEvidenceProtobufCodec.MESSAGE_TYPE &&
            descriptor.schemaDigestSha256.contentEquals(LeonaEvidenceProtobufCodec.descriptor.schemaDigestSha256)
