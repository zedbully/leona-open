...
void update_tamper_context(JNIEnv* env, jstring integrity_snapshot,
                           jstring tamper_policy_snapshot) {
...
Java_io_leonasec_leona_internal_runtime_OssNativeRuntime_init(
  ...
  leona::g_state.verbose =
      (leona::g_state.config_flags & leona::kFlagVerboseLog) != 0;
  update_tamper_context(env, integrity_snapshot, tamper_policy_snapshot);

  if (leona::g_state.verbose) {
    LOG_I(
        "leona init, flags=0x%llx integrity_blob_len=%zu "
        "tamper_policy_blob_len=%zu",
...
Java_io_leonasec_leona_internal_runtime_OssNativeRuntime_updateTamperContext(
    JNIEnv* env, jobject /*thiz*/, jstring integrity_snapshot,
    jstring tamper_policy_snapshot) {
  update_tamper_context(env, integrity_snapshot, tamper_policy_snapshot);
}
...
Java_io_leonasec_leona_internal_runtime_OssNativeRuntime_honeypotFakeToken(
  ...
  const auto bytes = leona::honeypot::fake_token_bytes(
      salt_vec,
      static_cast<size_t>(token_length_bytes > 0 ? token_length_bytes : 0));
